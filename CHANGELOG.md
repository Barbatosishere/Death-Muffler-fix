...
- **1.12.2 (Forge)**: Fixed dependency declaration — changed `mandatory = false` to `mandatory = true` for Mob Grinding Utils in `mcmod.info`. `ClientHandler` directly imports MUG capability classes and will throw `NoClassDefFoundError` on any boss bar render event if MUG is absent.
...
- **All versions**: Added missing `pack.mcmeta` resource pack metadata.
  ...
  - Pack formats: 1.12.2 → `3`, 1.20.1 → `15`, 1.21.1 → `34`.
...
- **1.12.2（Forge）**：修复依赖声明——在 `mcmod.info` 中将 Mob Grinding Utils 的 `mandatory` 从 `false` 改为 `true`。`ClientHandler` 直接引用 MUG 能力类，未安装 MUG 时任何 Boss 血条渲染事件都会抛出 `NoClassDefFoundError`。
...
- **全版本**：补充缺失的 `pack.mcmeta` 资源包元数据文件。
  ...
  - Pack format：1.12.2 → `3`，1.20.1 → `15`，1.21.1 → `34`。
